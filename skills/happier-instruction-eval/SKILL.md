...
Use the final response unless the user requested durable evaluation tracking. Do not create a new report file, update `AGENTS.md`, or propagate a variant to another repository without explicit change authority. If an accepted change affects the 0.2 source line, use `skills/happier-port-0-2-to-0-3` for its destination disposition.
